...
- **Decisión**: Se implementó un modelo de seguridad con cuatro capas:
  ...
  4. **Rate limiting del login (SlowAPI)**: Se aplican dos cuotas simultáneas: una por cuenta normalizada y hasheada, y otra por dirección de red directa. La primera evita que el proxy Streamlit agrupe incorrectamente a usuarios distintos por IP; la segunda evita eludir la protección rotando nombres de cuenta. No se confía en `X-Forwarded-For` sin una política explícita de proxies confiables.
...
- **Consecuencias**:
  ...
  - **Negativas**:
    ...
    - El almacenamiento de rate limiting es local al proceso por defecto; al escalar el backend a múltiples réplicas debe configurarse un backend compartido mediante `RATE_LIMIT_STORAGE_URI`.
...
- **Referencias**:
  ...
  - `pyproject.toml` — dependencias `bcrypt==5.0.0`, `PyJWT==2.13.0`, `slowapi==0.1.10`
